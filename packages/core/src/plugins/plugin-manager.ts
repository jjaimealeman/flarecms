...
import { getAppReference } from '../middleware/bootstrap'
...
export class PluginManager implements IPluginManager {
  ...
  constructor() {
    ...
    // Use the module-level singleton so plugin hooks share the same instance
    // as route handlers that call getHookSystem() to fire lifecycle events.
    this.hooks = getHookSystem()
  }
  ...
  async install(plugin: Plugin, config?: PluginConfig): Promise<void> {
    ...
    try {
      ...
      // Activate plugin — call plugin.activate(context) lifecycle callback
      // This is where plugins register Hono middleware/routes via pluginContext.app
      if (plugin.activate) {
        await plugin.activate(pluginContext)
      }

      // Update registry activation status
