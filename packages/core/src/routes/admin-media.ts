  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        let uploadBodyAdmin: ArrayBuffer | ReadableStream
        let arrayBufferAdmin: ArrayBuffer | null = null

        if (file.type.startsWith('image/') && !file.type.includes('svg')) {
          // Buffer images (typically < 10MB) to allow dimension extraction
          arrayBufferAdmin = await file.arrayBuffer()
          uploadBodyAdmin = arrayBufferAdmin
        } else {
          ...
          uploadBodyAdmin = file.stream()
        }

        const uploadResult = await c.env.MEDIA_BUCKET.put(r2Key, uploadBodyAdmin, {
          httpMetadata: {
        ...
        // Extract image dimensions if it's an image (arrayBufferAdmin is populated for images)
        let width: number | undefined
        let height: number | undefined

        if (arrayBufferAdmin !== null) {
          try {
            const dimensions = await getImageDimensions(arrayBufferAdmin)
