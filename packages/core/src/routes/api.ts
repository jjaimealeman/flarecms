...
// Check if cache plugin is active (inline query — isPluginActive import has bundling issues)
apiRoutes.use('*', async (c, next) => {
  ...
  try {
    const result = await c.env.DB
      ...
      .bind('core-cache')
      .first()
    cacheEnabled = result?.status === 'active'
  } catch {
    // Plugin table may not exist yet during bootstrap
  }
...
// Collection scope enforcement for scoped API tokens
// Blocks access to collection endpoints not in the token's allowed_collections list
apiRoutes.use('/collections/:collection/*', async (c, next) => {
  const apiToken = c.get('apiToken')
  if (!apiToken || !apiToken.allowed_collections) {
    return await next()
  }

  const requestedCollection = c.req.param('collection')
  if (!(apiToken.allowed_collections as string[]).includes(requestedCollection)) {
    log.info('content.access_denied', { collection: requestedCollection, userId: (apiToken as any).user_id, reason: 'token_scope' })
    return c.json({
      error: `Access denied: token is not authorized for collection '${requestedCollection}'`
    }, 403)
  }
