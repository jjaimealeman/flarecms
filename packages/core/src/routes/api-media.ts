...
apiMediaRoutes.post('/upload', async (c) => {
  try {
    ...
    // Upload to R2 — buffer images for dimension extraction, stream everything else
    let uploadBody: ArrayBuffer | ReadableStream
    let arrayBuffer: ArrayBuffer | null = null

    if (file.type.startsWith('image/') && !file.type.includes('svg')) {
      // Buffer images (typically < 10MB) to allow dimension extraction
      arrayBuffer = await file.arrayBuffer()
      uploadBody = arrayBuffer
    } else {
      // Stream non-image files (PDFs, videos, audio, docs) directly to R2
      // Avoids buffering large files into the 128MB Worker memory limit
    ...
    // After hook: media:after-upload (non-blocking via waitUntil)
    try {
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        // Upload to R2 — buffer images for dimension extraction, stream everything else
        let uploadBodyBulk: ArrayBuffer | ReadableStream
        let arrayBufferBulk: ArrayBuffer | null = null

        if (file.type.startsWith('image/') && !file.type.includes('svg')) {
          // Buffer images (typically < 10MB) to allow dimension extraction
          arrayBufferBulk = await file.arrayBuffer()
          uploadBodyBulk = arrayBufferBulk
        } else {
          // Stream non-image files (PDFs, videos, audio, docs) directly to R2
          // Avoids buffering large files into the 128MB Worker memory limit
          uploadBodyBulk = file.stream()
        }

        const uploadResult = await c.env.MEDIA_BUCKET.put(r2Key, uploadBodyBulk, {
          httpMetadata: {
            contentType: file.type,
            contentDisposition: `inline; filename="${file.name}"`
          },
        ...
        if (arrayBufferBulk !== null) {
          try {
            const dimensions = await getImageDimensions(arrayBufferBulk)
            width = dimensions.width
            height = dimensions.height
          } catch (error) {
            console.warn('Failed to extract image dimensions:', error)
    ...
    // After hook: media:after-upload (non-blocking via waitUntil) for batch
    if (uploadResults.length > 0) {
      try {
        ...
        console.error('[hooks] batch media upload hook failed:', hookErr)
      }
    }
...
apiMediaRoutes.post('/bulk-delete', async (c) => {
  try {
    ...
    // After hook: media:after-delete (non-blocking via waitUntil) for batch
    if (results.length > 0) {
      try {
        const hookSystem = getHookSystem()
        if (c.executionCtx) {
          c.executionCtx.waitUntil(
            hookSystem.execute(HOOKS.AFTER_MEDIA_DELETE, {
        ...
        console.error('[hooks] batch media delete hook failed:', hookErr)
      }
    }
...
apiMediaRoutes.post('/bulk-move', async (c) => {
  try {
    ...
    // After hook: media:after-move (non-blocking via waitUntil)
    // Note: 'media:after-move' is not in the 7 named HOOKS constants — use string directly
    if (results.length > 0) {
      try {
        const hookSystem = getHookSystem()
        if (c.executionCtx) {
          c.executionCtx.waitUntil(
            hookSystem.execute('media:after-move', {
              count: results.length,
              targetFolder,
              ids: fileIds,
            }),
        ...
        console.error('[hooks] bulk media move hook failed:', hookErr)
      }
    }
...
apiMediaRoutes.delete('/:id', async (c) => {
  try {
    ...
    // After hook: media:after-delete (non-blocking via waitUntil)
    try {
      const hookSystem = getHookSystem()
      if (c.executionCtx) {
        c.executionCtx.waitUntil(
      ...
      console.error('[hooks] media delete hook failed:', hookErr)
    }
...
apiMediaRoutes.patch('/:id', async (c) => {
  try {
    ...
    // After hook: media:after-update (non-blocking via waitUntil)
    // Note: 'media:after-update' is not in the 7 named HOOKS constants — use string directly
    try {
      const hookSystem = getHookSystem()
      if (c.executionCtx) {
        c.executionCtx.waitUntil(
          hookSystem.execute('media:after-update', {
            id: fileId,
          }),
        )
      }
    } catch (hookErr) {
      console.error('[hooks] media update hook failed:', hookErr)
    }
