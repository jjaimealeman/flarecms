...
// Supports both JWT (cookie/Authorization header) and API tokens (X-API-Key header)
export const requireAuth = () => {
  return async (c: Context, next: Next) => {
    try {
      // --- API Key authentication via X-API-Key header ---
      // Checked first so API tokens work on endpoints that use requireAuth
      ...
      if (apiKey) {
        ...
        if (db) {
          ...
          if (result.valid && result.tokenRecord) {
            ...
            // Store apiToken info if the Variables type supports it
        ...
        // Invalid apiToken provided — reject immediately
        return c.json({ error: 'Invalid API key' }, 401)
      }

      // --- JWT authentication (Authorization header / cookie) ---
...
export const requireRole = (requiredRole: string | string[]) => {
  return async (c: Context, next: Next) => {
    ...
    if (!roles.includes(user.role)) {
      log.warn('auth.permission_denied', { userId: user.userId, resource: c.req.path, method: c.req.method, requiredRoles: roles, userRole: user.role })
      // Check if this is a browser request (HTML accept header)
      const acceptHeader = c.req.header('Accept') || ''
      if (acceptHeader.includes('text/html')) {
