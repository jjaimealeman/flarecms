...
import { grantCollectionPermission, revokeCollectionPermission, getCollectionPermissions } from '../services/rbac'
...
// Profile and activity log routes above this point are available to all authenticated users
 ...
 * Protected by admin requireRole middleware above.
 */
...
userRoutes.get('/users/:id/edit', async (c) => {
  ...
  try {
    ...
    const pageData: UserEditPageData = {
      userToEdit: editData,
      roles: ROLES,
      showCollectionPermissions: true,
      user: {
        name: user!.email.split('@')[0] || user!.email,
        email: user!.email,
        role: user!.role
...
/**
 * GET /admin/users/:id/collection-permissions
 * HTMX fragment: render collection permissions management table for a user.
 * Returns a table of all collections with their current role (or none).
 * Protected by admin requireRole middleware via /users/* pattern.
 */
userRoutes.get('/users/:id/collection-permissions', async (c) => {
  const db = c.env.DB
  const userId = c.req.param('id')
  ...
  try {
    ...
    const { results: collections } = await db.prepare(
      'SELECT id, name, display_name FROM collections WHERE is_active = 1 ORDER BY display_name'
    ).all()

    // Load current permissions for this user
    const currentPerms = await getCollectionPermissions(db, userId)
    const permMap = new Map(currentPerms.map(p => [p.collectionId, p.role]))

    const rows = (collections || []).map((col: any) => {
      const currentRole = permMap.get(col.id) ?? ''
      return `
        <tr class="border-b border-zinc-100 dark:border-zinc-800">
          <td class="py-3 px-4 text-sm text-zinc-900 dark:text-zinc-100">${col.display_name || col.name}</td>
          <td class="py-3 px-4">
            <select
              name="role"
              hx-post="/admin/users/${userId}/collection-permissions"
              hx-vals='{"collectionId": "${col.id}", "role": ""}'
              hx-trigger="change"
              hx-target="#perm-${col.id}"
              hx-include="[name='role']"
              hx-swap="outerHTML"
              id="perm-select-${col.id}"
              class="rounded-md border-0 py-1.5 px-3 text-sm text-zinc-900 dark:text-zinc-100 bg-white dark:bg-zinc-800 ring-1 ring-inset ring-zinc-300 dark:ring-zinc-600 focus:ring-2 focus:ring-inset focus:ring-blue-600"
              onchange="this.setAttribute('hx-vals', JSON.stringify({collectionId: '${col.id}', role: this.value})); htmx.trigger(this, 'change');"
            >
              <option value="" ${currentRole === '' ? 'selected' : ''}>— No access —</option>
              <option value="viewer" ${currentRole === 'viewer' ? 'selected' : ''}>Viewer (read-only)</option>
              <option value="author" ${currentRole === 'author' ? 'selected' : ''}>Author (own content)</option>
              <option value="editor" ${currentRole === 'editor' ? 'selected' : ''}>Editor (all content)</option>
            </select>
          </td>
          <td class="py-3 px-4 text-sm" id="perm-${col.id}">
            ${currentRole ? `<span class="inline-flex items-center rounded-md bg-green-50 dark:bg-green-500/10 px-2 py-1 text-xs font-medium text-green-700 dark:text-green-400 ring-1 ring-inset ring-green-600/20">${currentRole}</span>` : '<span class="text-zinc-400 dark:text-zinc-500 text-xs">none</span>'}
          </td>
        </tr>
      `
    }).join('')
    ...
    return c.html(`
      <div id="collection-permissions-table">
        <table class="min-w-full">
          ...
          <tbody>
            ${rows || '<tr><td colspan="3" class="py-4 px-4 text-sm text-zinc-400">No collections found.</td></tr>'}
          </tbody>
        </table>
        <p class="mt-3 text-xs text-zinc-500 dark:text-zinc-400">Changes are saved immediately on role selection. Only applies to non-admin users.</p>
      </div>
    `)
  } catch (error) {
    console.error('Collection permissions load error:', error)
    return c.html('<p class="text-red-500 text-sm">Failed to load collection permissions.</p>', 500)
  }
})

/**
 * POST /admin/users/:id/collection-permissions
 * HTMX handler: grant or revoke a collection permission for a user.
 * Body: collectionId, role (empty string = revoke)
 */
userRoutes.post('/users/:id/collection-permissions', async (c) => {
  const db = c.env.DB
  ...
  try {
    ...
    if (!collectionId) {
      return c.html('<span class="text-red-500 text-xs">Missing collectionId</span>', 400)
    }

    if (!role) {
      // Empty role = revoke permission
      await revokeCollectionPermission(db, userId, collectionId)
      return c.html('<span class="text-zinc-400 dark:text-zinc-500 text-xs">none</span>')
    }

    await grantCollectionPermission(db, {
      userId,
      collectionId,
      role,
    ...
    return c.html(`<span class="inline-flex items-center rounded-md bg-green-50 dark:bg-green-500/10 px-2 py-1 text-xs font-medium text-green-700 dark:text-green-400 ring-1 ring-inset ring-green-600/20">${role}</span>`)
  } catch (error) {
    console.error('Collection permission update error:', error)
    return c.html('<span class="text-red-500 text-xs">Failed to update permission</span>', 500)
  }
})
