...
import { HookSystem, HookHandler, HookExecutionResult, PluginHook, HookContext } from '../types'
...
export class HookSystemImpl implements HookSystem {
   ...
   * Delegates to executeWithResult() for backward compatibility
   */
  async execute(hookName: string, data: any, context?: any): Promise<any> {
    const result = await this.executeWithResult(hookName, data, context)
    return result.data
  }

  /**
   * Execute all handlers for a hook and return cancellation status
   * Use this when callers need to know if the hook chain was cancelled
   */
...
export class ScopedHookSystem {
  ...
  /**
   * Execute a hook and return cancellation status
   */
  async executeWithResult(hookName: string, data: any, context?: any): Promise<HookExecutionResult> {
    return this.parent.executeWithResult(hookName, data, context)
  }
