...
export class QueryFilterBuilder {
  ...
  static parseFromQuery(query: Record<string, any>): QueryFilter {
    ...
    // Parse bracket-syntax filter params: filter[field][operator]=value
    ...
    for (const [key, value] of Object.entries(query)) {
      ...
      if (operator && field && filter.where && filter.where.and) {
    ...
    // Parse sort
    if (query.sort) {
      // Support shorthand string sort: "created_at" or "-created_at" (descending)
      // A shorthand sort is a simple field name: optional leading "-", then alphanumeric/underscore only
      const shorthandSortPattern = /^-?[a-zA-Z_][a-zA-Z0-9_]*$/
      if (typeof query.sort === 'string' && shorthandSortPattern.test(query.sort)) {
        const sortField = query.sort
        if (sortField.startsWith('-')) {
          filter.sort = [{ field: sortField.slice(1), order: 'desc' }]
        } else {
          filter.sort = [{ field: sortField, order: 'asc' }]
        }
      } else {
        try {
